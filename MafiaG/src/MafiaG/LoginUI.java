...
public void showLoginUI() {
    ...
    loginBtn.addActionListener(e -> {
        ...
        if (success) {
            ...
            try {
                // 서버 실행 - Server 클래스 main() 호출
                ProcessBuilder pb = new ProcessBuilder("java", "MafiaG.Server");
                pb.inheritIO(); // 콘솔 출력 같이 보기
                pb.start();
            } catch (IOException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(null, "서버 실행에 실패했습니다.");
            }
